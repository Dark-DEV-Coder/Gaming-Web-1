<?php       
    ...
    if (isset($_GET['tkma']) && isset($_GET['tkgt']) && isset($_GET['tkhoten'])){
        ...
        if ($tkma=="" && $tkhoten=="" && $tkgt==-1){
            $sql0="select * from khachhang where TinhTrang=0 limit $trang,$den";
            $sql01="select * from khachhang where TinhTrang=0";
            $re0=$p->Check($sql0);
            $re01=$p->Check($sql01);
        ...
        if ($tkma!="" && $tkhoten=="" && $tkgt==-1){
            $sql1="select * from khachhang where TinhTrang=0 AND MaKH='$tkma'";
            $re1=$p->Check($sql1);
        ...
        if ($tkma=="" && $tkhoten=="" && $tkgt!=-1){
            if ($tkgt==0){
                $sql2="select * from khachhang where TinhTrang=0 AND GioiTinh=0 limit $trang,$den";
                $sql21="select * from khachhang where TinhTrang=0 AND GioiTinh=0";
            }
            if ($tkgt==1){
                $sql2="select * from khachhang where TinhTrang=0 AND GioiTinh=1 limit $trang,$den";
                $sql21="select * from khachhang where TinhTrang=0 AND GioiTinh=1";
            }
            $re2=$p->Check($sql2);
            $re21=$p->Check($sql21);
        ...
        if ($tkma=="" && $tkhoten!="" && $tkgt==-1){            
            $sql3="select * from khachhang where TinhTrang=0 limit $trang,$den";            
            $re3=$p->Check($sql3);
            $sql31="select * from khachhang where TinhTrang=0";            
            $re31=$p->Check($sql31);
        ...
        if ($tkma=="" && $tkhoten!="" && $tkgt!=-1){
            if ($tkgt==0){
                $sql4="select * from khachhang where TinhTrang=0 AND GioiTinh=0 limit $trang,$den";
                $sql41="select * from khachhang where TinhTrang=0 AND GioiTinh=0";
            }
            if ($tkgt==1){
                $sql4="select * from khachhang where TinhTrang=0 AND GioiTinh=1 limit $trang,$den";
                $sql41="select * from khachhang where TinhTrang=0 AND GioiTinh=1";
            }
            $re4=$p->Check($sql4);
            $re41=$p->Check($sql41);
        ...
        if ($d1>1){
            ...
            for ($i=0;$i<$d1;$i++){
                $l=$i+1;
                if ($l==$idpt)
                    $spt=$spt."<div p='$l' style='background-color:red'>$l</div>";
                else
                    $spt=$spt."<div p='$l'>$l</div>";
            }
            $spt=$spt."</div></div>";
        }
        echo $s.$spt;
...
<script>
        $(document).ready(function(){
            ...
            $(".con6-row div").click(function(){
                ...
                var gt=$("#tk-gtkh").val(); 
                $.get("./timkiemkh.php",{id:'KH',idpt:pt,tkma:ma,tkgt:gt,tkhoten:hoten},function(data){
                    $("#kh-pt").html(data);
                });
            });            
        });
